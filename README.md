...
```
anton-voice-agent/
├── server.py           # uv run anton  → starts the MCP server (SSE on :8000)
├── agent_anton.py     # uv run anton_voice → starts the LiveKit voice agent
├── pyproject.toml
├── .env.example        # copy → .env and fill in your keys
│
└── anton/             # MCP server package
    ...
    └── resources/      # MCP resources exposed to clients (anton://info)
```
...
```bash
git clone https://github.com/SAGAR-TAMANG/anton-voice-agent.git
...
Starts the LiveKit voice agent in **dev mode** — it joins a LiveKit room and begins listening. Open the [LiveKit Agents Playground](https://agents-playground.livekit.io) and connect to your room to talk to Anton.

---

## `uv run anton` vs `uv run anton_voice`

| Command | Entry point | What it does |
|---------|------------|--------------|
| `uv run anton` | `server.py → main()` | Launches the **FastMCP server** over SSE transport on port 8000. This is the "brain backend" — it registers all tools, prompts, and resources that the LLM can call. |
| `uv run anton_voice` | `agent_anton.py → dev()` | Launches the **LiveKit voice agent**. It builds the STT / LLM / TTS pipeline, connects to your LiveKit room, and wires up the MCP server as a tool source. The `dev()` wrapper auto-injects the `dev` CLI flag so you don't have to type it manually. |
...
Open `agent_anton.py` and change the provider constants at the top:

```python
STT_PROVIDER = "sarvam"   # "sarvam" | "whisper"
LLM_PROVIDER = "openai"   # "openai" (default, uses GPT-4o)
...
1. Create or open a file in `anton/tools/`
2. Define a `register(mcp)` function and decorate tools with `@mcp.tool()`
3. Import and call `register(mcp)` inside `anton/tools/__init__.py`
